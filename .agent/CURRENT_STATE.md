...
- MTS Jobs adapter: Product JSON-LD parser with archive detection (`offers.availability: OutOfStock` → definitive rejection). URL shape `https://job.mts.ru/vacancy/{numeric_id}`. 3 parsing tests (valid, archive, missing Product), 4 ingestion tests (valid, 404, 410, archived 200, 503, oversized, redirect). Full suite: 721 tests, typecheck clean, build clean. Live probe is unstable — job.mts.ru archives vacancies shortly after posting; all sitemap IDs return HTTP 200 with Product JSON-LD `InStock` but also show an archive modal with "Спасибо за отклик". Seed URL is a URL-shape example; service stays `pending` by default.
