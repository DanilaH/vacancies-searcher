...
- Full suite passed with 623 tests after fuzzy dedup report PR #21 (`npm test`).
...
- Owner-only `/fuzzyreport` command: aggregates fuzzy duplicate stats for the last 30 days via `getFuzzyDedupStats(sinceIso)` in database layer. Reports total links, unique groups (connected components via union-find), average/min/max score, score buckets, top source/channel pairs, group size distribution, and last match date. `buildFuzzyDedupReport` formats the output. `handleFuzzyDedupReportCommand` enforces owner-only access. 16 tests cover aggregation, groups, buckets, time filtering, source ranking, access control, and privacy (no vacancy text, contacts, or user IDs).
