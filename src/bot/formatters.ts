...
export function formatStartMessage(_config: AppConfig): string {
  return [
    ...
    "",
    "Чтобы начать получать подходящие вакансии, открой «Мои поиски» и настрой первый поиск."
  ].join("\n");
}
...
export function formatOnboardingSetupChoiceMessage(): string {
  return [
    ...
    "",
    "Позже всё можно поменять в разделе «Мои поиски» и добавить до пяти отдельных поисков."
  ].join("\n");
}
...
export function formatOnboardingLanguageMessage(mode: VacancyLanguageMode): string {
  return [
    ...
    "Выбери язык текущего поиска. Это можно поменять позже в разделе «Мои поиски».",
...
export function formatOnboardingCompletionMessage(
  ...
  if (options.trigger === "skipped") {
    return [
      ...
      health.guidance,
      "",
      "Когда будешь готов, открой «Мои поиски» и заполни первый поиск."
  ...
  const resultLines = !health.isSearchActive
    ? [
        ...
        "Заполни обязательные блоки в «Мои поиски», и бот сразу пересоберёт подборку."
      ]
    : options.initialMatchesCount > 0
      ...
      : [
          ...
          "Новые подходящие вакансии придут автоматически. Если захочется расширить выдачу, проверь «Мои поиски»."
        ];
...
function buildQuietDiagnosticsHints(payload: UserQuietDiagnosticsPayload): string[] {
  ...
  if (!payload.health.isSearchActive) {
    hints.push("заполнить обязательные блоки в «Мои поиски»");
  }
...
export function formatBlockedWeeklyAccess(
  ...
  return [
    ...
    "",
    "Перейдите в «Мои поиски», чтобы настроить профиль."
  ].join("\n");
}
