...
export function createUserPanels(deps: UserPanelsDeps): UserPanels {
  ...
  function buildStartProfileNotice(currentUserId: string | null): string | null {
    ...
    if (healthReports.some((health) => health.status === "weak")) {
      return [
        ...
        "Заполни обязательные блоки в разделе «Мои поиски», чтобы открыть подборку вакансий за неделю."
      ].join("\n");
    }

    return [
      "🔴 Поиск пока неактивен.",
      "Заполни профиль в разделе «Мои поиски» или выбери готовый пресет, чтобы открыть подборку и получать подходящие вакансии."
    ].join("\n");
  }
  ...
  async function showNotificationsPanel(ctx: Context, mode: BotPanelMode = "reply"): Promise<void> {
    ...
    await replyOrEdit(ctx, mode, formatNotificationPreferences(settings?.notifyOnEmptyCycle ?? false, settings?.dailyDigestEnabled ?? false, settings?.instantVacancyNotificationsEnabled ?? true, settings?.notificationQuietHoursEnabled ?? false), {
      reply_markup: createNotificationsKeyboard(settings?.notifyOnEmptyCycle ?? false, settings?.dailyDigestEnabled ?? false, settings?.instantVacancyNotificationsEnabled ?? true, settings?.notificationQuietHoursEnabled ?? false)
    });
  }
